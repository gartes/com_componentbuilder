...
/**
 * Copmare_list Table class
 */
class Vm_product_comparisonsTableCopmare extends JTable
{
	/**
	...
	{
		parent::__construct('#__vm_product_comparisons_copmare', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Copmare table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New copmare. A copmare created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('copmare', 'Vm_product_comparisonsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_VM_PRODUCT_COMPARISONS_COPMARE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_vm_product_comparisons.copmare.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_vm_product_comparisons.copmare.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_vm_product_comparisons');
