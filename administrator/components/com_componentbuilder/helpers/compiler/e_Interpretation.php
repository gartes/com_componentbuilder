...
//				echo'<pre>';print_r( $this->componentData->version_update );echo'</pre>'.__FILE__.' '.__LINE__;
	...
	/**
	 * Постраитель Update XML для каждой версии Компонента
	 * set Update XML SQL
	 *
	 * @param array $update
	 * @param array $updateXML
	...
	public function setUpdateXMLSQL(&$update, &$updateXML, &$addDynamicSQL)
	{
		...
		# убедитесь в правильности именования версий
	...
	public function noHelp()
	{
		...
		$help[] = $this->_t(1) . " *	Может использоваться для создания справочных URL.";
		$help[] = $this->_t(1) . " *	Can be used to build help urls.";
		$help[] = $this->_t(1) . " *	@since 3.9 ";
		$help[] = $this->_t(1) . " **/";
	...
	public function setExelHelperMethods()
	{
		if ($this->addEximport)
		{
			...
			$exel[] = $this->_t(1) . " * Готовит документ XML";
			$exel[] = $this->_t(1) . " * Prepares the xml document";
			$exel[] = $this->_t(1) . " * @since 3.9";
			$exel[] = $this->_t(1) . " */";
...
//			$allow[] = $this->_t(3) . "}";
	...
	public function setAutoCheckin($view, $component)
	{
		...
		$checkin .= PHP_EOL . $this->_t(1) . " * Создает SQL-запрос для проверки всех элементов, оставленных извлеченными, дольше установленного времени.";
		...
		$checkin .= PHP_EOL . $this->_t(1) . " *";
		$checkin .= PHP_EOL . $this->_t(1) . " * @since 3.9";
		$checkin .= PHP_EOL . $this->_t(1) . " *";
		$checkin .= PHP_EOL . $this->_t(1) . " */";
...
function vdm_dkim() {
	...
	/**
	 * Создание файла манифеста плагина
	 * @param $plugin
	 *
	 * @return string
	 * @since 3.9
	...
	protected function setInstallMethodScript($function_name, &$scripts)
	{
		...
		switch($function_name)
		{
			...
			case 'postflight':
				...
				$script .= PHP_EOL . $this->_t(1) .' *';
				$script .= PHP_EOL . $this->_t(1) .' * @return  boolean  True on success';
				$script .= PHP_EOL . $this->_t(1) .' * @throws Exception';
				$script .= PHP_EOL . $this->_t(1) .' * @since 3.9';
				$script .= PHP_EOL . $this->_t(1) .' */';
