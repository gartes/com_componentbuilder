...
//			echo'<pre>$localPath :: ';print_r( $localPath );echo'</pre>'.__FILE__.' '.__LINE__;
//			echo'<pre>$fileName :: ';print_r( $fileName );echo'</pre>'.__FILE__.' '.__LINE__;
	...
	**/
	...
	public static function getFtp($serverID, $permission)
	{
		...
		if ($server = self::getServerDetails($serverID, 1, $permission))
		{
			...
			{
				// turn into variables
				// because of this I am using strange variable naming to avoid any collisions.
				parse_str($server->signature , $options);
				// set options
...
//				die(__FILE__ .' Lines '. __LINE__ );
				...
				{
					// heads-up on protocol
					// FTP <-- if called not knowing what type of protocol is being used
					self::$ftp[$server->cache]->jcb_protocol = 1;
					// set the unique buckets
